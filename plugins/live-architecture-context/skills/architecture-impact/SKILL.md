...
1. Run `archctx status` when `.archctx/architecture.json` exists (or the exact configured command from local guidance); if it is `FRESH`, run `impact --files <exact changed paths>`.
